-- Adicionar foreign keys e relacionamentos entre tabelas
ALTER TABLE production_records 
ADD CONSTRAINT fk_production_records_machine_id 
FOREIGN KEY (machine_id) REFERENCES machines(id) ON DELETE CASCADE;

ALTER TABLE oee_history 
ADD CONSTRAINT fk_oee_history_machine_id 
FOREIGN KEY (machine_id) REFERENCES machines(id) ON DELETE CASCADE;

ALTER TABLE downtime_events 
ADD CONSTRAINT fk_downtime_events_machine_id 
FOREIGN KEY (machine_id) REFERENCES machines(id) ON DELETE CASCADE;

ALTER TABLE alerts 
ADD CONSTRAINT fk_alerts_machine_id 
FOREIGN KEY (machine_id) REFERENCES machines(id) ON DELETE CASCADE;

-- Adicionar constraint única para production_records por máquina (apenas um registro ativo por máquina)
ALTER TABLE production_records 
ADD CONSTRAINT unique_active_production_per_machine 
UNIQUE (machine_id);

-- Adicionar coluna downtime_reason_id em downtime_events para relacionar com downtime_reasons
ALTER TABLE downtime_events 
ADD COLUMN downtime_reason_id UUID REFERENCES downtime_reasons(id) ON DELETE SET NULL;

-- Criar função para calcular OEE automaticamente
...
-- Criar função para upsert de production_records
...
-- Criar função para deletar máquina e todos os registros relacionados
